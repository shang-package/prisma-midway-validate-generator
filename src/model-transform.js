"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.transformModelList = void 0;
const enum_helper_1 = require("./enum-helper");
const format_1 = require("./format");
const import_helper_1 = require("./import-helper");
function transformType({ isList, type, location }, action) {
    let baseType = "";
    let baseJoi = "";
    switch (type) {
        case "String":
            baseType = "string";
            baseJoi = `RuleType.string()`;
            break;
        case "Int":
        case "Float":
            baseType = "number";
            baseJoi = `RuleType.number()`;
            break;
        case "Boolean":
            baseType = "boolean";
            baseJoi = `RuleType.boolean()`;
            break;
        case "DateTime":
            baseType = "Date";
            baseJoi = `RuleType.date()`;
            break;
        case "Json":
            baseType = "any";
            baseJoi = "RuleType.any()";
            break;
        default:
            switch (location) {
                case "enumTypes":
                    const enumName = typeof type === "string" ? type : type.name;
                    const values = enum_helper_1.enumHelper.getValues(enumName);
                    baseType = enumName;
                    baseJoi = `RuleType.string().valid(...${JSON.stringify(values)})`;
                    import_helper_1.importHelper.add("@prisma/client", enumName);
                    break;
                case "outputObjectTypes":
                    baseType = type;
                    baseJoi = `${type}${action}DTO`;
                    import_helper_1.importHelper.add("@prisma/client", baseType);
                    break;
                default:
                    baseType = "unknown";
                    baseJoi = `RuleType.any()`;
                    break;
            }
            break;
    }
    if (isList && baseJoi.endsWith("DTO")) {
        // importHelper.add("@midwayjs/validate", "getSchema");
        // baseJoi = `getSchema(${baseJoi})`;
        return;
    }
    return {
        type: isList ? `${baseType}[]` : baseType,
        joi: isList ? `RuleType.array().items(${baseJoi})` : baseJoi,
    };
}
function transformCreateModel(model) {
    const { name, fields } = model;
    const str = fields
        .map(({ name, isNullable, outputType }) => {
        if (name === "id") {
            return;
        }
        if (name.startsWith("_")) {
            return;
        }
        const vv = transformType(outputType, "Create");
        if (!vv) {
            return;
        }
        const { type, joi } = vv;
        if (outputType.location === "outputObjectTypes") {
            return `
@Rule(${joi})
${name}${isNullable ? "?" : ""}: ${type}
`;
        }
        return `
@Rule(${joi}${isNullable ? "" : ".required()"})
${name}${isNullable ? "?" : ""}: ${type}
`;
    })
        .filter((v) => {
        return !!v;
    })
        .join("\n\n");
    return `
export class ${name}CreateDTO {
  ${str}
}`;
}
function transformUpdateModel(model) {
    const { name, fields } = model;
    const str = fields
        .map(({ name, outputType }) => {
        if (name === "id") {
            return;
        }
        if (name.startsWith("_")) {
            return;
        }
        const vv = transformType(outputType, "Update");
        if (!vv) {
            return;
        }
        const { type, joi } = vv;
        return `
      @Rule(${joi})
      ${name}?: ${type}
      `;
    })
        .filter((v) => {
        return !!v;
    })
        .join("\n\n");
    return `
export class ${name}UpdateDTO {
  ${str}
}`;
}
async function transformModelList(modelList) {
    import_helper_1.importHelper.add("@midwayjs/validate", "Rule", "RuleType");
    const modelTypes = modelList
        .map((model) => {
        return [transformCreateModel(model), transformUpdateModel(model)];
    })
        .flat()
        .sort((v1, v2) => {
        let nu1 = v1.match(/@Rule\(\s*(?!RuleType)/)?.length ?? 0;
        let nu2 = v2.match(/@Rule\(\s*(?!RuleType)/)?.length ?? 0;
        return nu1 - nu2;
    })
        .join("\n\n\n\n");
    const result = await (0, format_1.formatCode)(`
    ${import_helper_1.importHelper.toString()}

    ${modelTypes}
    `);
    return result;
}
exports.transformModelList = transformModelList;
//# sourceMappingURL=model-transform.js.map